...
public:
   ...
   void improve(StateValueFunction s,vector<vector<double>> rw,vector<vector<bool>> wall);
...
public:
   ...
   void improve(const vector<vector<double>> rw,const vector<vector<bool>> wall,const pair<int,int> goal);
...
void Policy::improve(StateValueFunction s,vector<vector<double>> rw,vector<vector<bool>> wall){
...
void StateValueFunction::improve(const vector<vector<double>> rw,const vector<vector<bool>> wall,const pair<int,int> goal){
   double delta=1;
   while (delta>EPS){
      delta=0;
      vector<vector<double>> newsvf(height,vector<double>(width,0));
...
int main(){
   ...
   int N=3;
   for (int i=0;i<N;i++){
      s.improve(reward,wall,goal);
      p.improve(s,reward,wall);
   }
   s.show();
   p.show(wall,goal);
}
